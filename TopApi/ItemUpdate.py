class ItemUpdate(object):
    def __init__(self):
        self.apiParas={'method':'taobao.item.update'}
    def setAfterSaleId(self,afterSaleId):
        self.apiParas['after_sale_id']=afterSaleId
    def getAfterSaleId(self):
        return self.apiParas['after_sale_id']
    def setApproveStatus(self,approveStatus):
        self.apiParas['approve_status']=approveStatus
    def getApproveStatus(self):
        return self.apiParas['approve_status']
    def setAuctionPoint(self,auctionPoint):
        self.apiParas['auction_point']=auctionPoint
    def getAuctionPoint(self):
        return self.apiParas['auction_point']
    def setAutoFill(self,autoFill):
        self.apiParas['auto_fill']=autoFill
    def getAutoFill(self):
        return self.apiParas['auto_fill']
    def setCid(self,cid):
        self.apiParas['cid']=cid
    def getCid(self):
        return self.apiParas['cid']
    def setCodPostageId(self,codPostageId):
        self.apiParas['cod_postage_id']=codPostageId
    def getCodPostageId(self):
        return self.apiParas['cod_postage_id']
    def setDesc(self,desc):
        self.apiParas['desc']=desc
    def getDesc(self):
        return self.apiParas['desc']
    def setEmsFee(self,emsFee):
        self.apiParas['ems_fee']=emsFee
    def getEmsFee(self):
        return self.apiParas['ems_fee']
    def setExpressFee(self,expressFee):
        self.apiParas['express_fee']=expressFee
    def getExpressFee(self):
        return self.apiParas['express_fee']
    def setFreightPayer(self,freightPayer):
        self.apiParas['freight_payer']=freightPayer
    def getFreightPayer(self):
        return self.apiParas['freight_payer']
    def setHasDiscount(self,hasDiscount):
        self.apiParas['has_discount']=hasDiscount
    def getHasDiscount(self):
        return self.apiParas['has_discount']
    def setHasInvoice(self,hasInvoice):
        self.apiParas['has_invoice']=hasInvoice
    def getHasInvoice(self):
        return self.apiParas['has_invoice']
    def setHasShowcase(self,hasShowcase):
        self.apiParas['has_showcase']=hasShowcase
    def getHasShowcase(self):
        return self.apiParas['has_showcase']
    def setHasWarranty(self,hasWarranty):
        self.apiParas['has_warranty']=hasWarranty
    def getHasWarranty(self):
        return self.apiParas['has_warranty']
    def setImage(self,image):
        self.apiParas['image']=image
    def getImage(self):
        return self.apiParas['image']
    def setIncrement(self,increment):
        self.apiParas['increment']=increment
    def getIncrement(self):
        return self.apiParas['increment']
    def setInputPids(self,inputPids):
        self.apiParas['input_pids']=inputPids
    def getInputPids(self):
        return self.apiParas['input_pids']
    def setInputStr(self,inputStr):
        self.apiParas['input_str']=inputStr
    def getInputStr(self):
        return self.apiParas['input_str']
    def setIs3D(self,is3D):
        self.apiParas['is_3D']=is3D
    def getIs3D(self):
        return self.apiParas['is_3D']
    def setIsEx(self,isEx):
        self.apiParas['is_ex']=isEx
    def getIsEx(self):
        return self.apiParas['is_ex']
    def setIsReplaceSku(self,isReplaceSku):
        self.apiParas['is_replace_sku']=isReplaceSku
    def getIsReplaceSku(self):
        return self.apiParas['is_replace_sku']
    def setIsTaobao(self,isTaobao):
        self.apiParas['is_taobao']=isTaobao
    def getIsTaobao(self):
        return self.apiParas['is_taobao']
    def setLang(self,lang):
        self.apiParas['lang']=lang
    def getLang(self):
        return self.apiParas['lang']
    def setListTime(self,listTime):
        self.apiParas['list_time']=listTime
    def getListTime(self):
        return self.apiParas['list_time']
    def setLocationCity(self,locationCity):
        self.apiParas['location.city']=locationCity
    def getLocationCity(self):
        return self.apiParas['location.city']
    def setLocationState(self,locationState):
        self.apiParas['location.state']=locationState
    def getLocationState(self):
        return self.apiParas['location.state']
    def setNum(self,num):
        self.apiParas['num']=num
    def getNum(self):
        return self.apiParas['num']
    def setNumIid(self,numIid):
        self.apiParas['num_iid']=numIid
    def getNumIid(self):
        return self.apiParas['num_iid']
    def setOuterId(self,outerId):
        self.apiParas['outer_id']=outerId
    def getOuterId(self):
        return self.apiParas['outer_id']
    def setPicPath(self,picPath):
        self.apiParas['pic_path']=picPath
    def getPicPath(self):
        return self.apiParas['pic_path']
    def setPostFee(self,postFee):
        self.apiParas['post_fee']=postFee
    def getPostFee(self):
        return self.apiParas['post_fee']
    def setPostageId(self,postageId):
        self.apiParas['postage_id']=postageId
    def getPostageId(self):
        return self.apiParas['postage_id']
    def setPrice(self,price):
        self.apiParas['price']=price
    def getPrice(self):
        return self.apiParas['price']
    def setProductId(self,productId):
        self.apiParas['product_id']=productId
    def getProductId(self):
        return self.apiParas['product_id']
    def setPropertyAlias(self,propertyAlias):
        self.apiParas['property_alias']=propertyAlias
    def getPropertyAlias(self):
        return self.apiParas['property_alias']
    def setProps(self,props):
        self.apiParas['props']=props
    def getProps(self):
        return self.apiParas['props']
    def setSellPromise(self,sellPromise):
        self.apiParas['sell_promise']=sellPromise
    def getSellPromise(self):
        return self.apiParas['sell_promise']
    def setSellerCids(self,sellerCids):
        self.apiParas['seller_cids']=sellerCids
    def getSellerCids(self):
        return self.apiParas['seller_cids']
    def setSkuOuterIds(self,skuOuterIds):
        self.apiParas['sku_outer_ids']=skuOuterIds
    def getSkuOuterIds(self):
        return self.apiParas['sku_outer_ids']
    def setSkuPrices(self,skuPrices):
        self.apiParas['sku_prices']=skuPrices
    def getSkuPrices(self):
        return self.apiParas['sku_prices']
    def setSkuProperties(self,skuProperties):
        self.apiParas['sku_properties']=skuProperties
    def getSkuProperties(self):
        return self.apiParas['sku_properties']
    def setSkuQuantities(self,skuQuantities):
        self.apiParas['sku_quantities']=skuQuantities
    def getSkuQuantities(self):
        return self.apiParas['sku_quantities']
    def setStuffStatus(self,stuffStatus):
        self.apiParas['stuff_status']=stuffStatus
    def getStuffStatus(self):
        return self.apiParas['stuff_status']
    def setTitle(self,title):
        self.apiParas['title']=title
    def getTitle(self):
        return self.apiParas['title']
    def setValidThru(self,validThru):
        self.apiParas['valid_thru']=validThru
    def getValidThru(self):
        return self.apiParas['valid_thru']
    def getApiMethodName(self):
        return self.apiParas['method']
    def getApiParas(self):
        return self.apiParas
